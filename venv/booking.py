...
for l in range(1, 6):
    ...
    tomorrow = time.localtime(time.time() + 24*3600 * l)
    ...
    for j in range(0, 3):
        try:
            with open('booking.csv', 'r') as file1:
        ...
        while flagcount <= 3:
            ...
            with open('booking.csv', mode='a', newline='') as employee_file:
                ...
                for i in range(num_of_blocks):
                    ...
                    try:
                        if[todaysDate,tomorrowsDate,hotelName.text,re.sub('([^0-9,])+', '', hotelPrice.text),hotelRating.text + "/10",re.sub('([^0-9,])+', '',hotelReviews.text)] not in existingLines:
